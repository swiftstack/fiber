...
@MainActor
@Test("FiberLoop.current")
func fiberLoopCurrent() {
    #expect((FiberLoop.current as Any) is FiberLoop)
    #expect(FiberLoop.main == FiberLoop.current)
}

@Test("FiberLoop.main != FiberLoop.current")
func evenLoopAnotherThread() {
    var tested = false
...
@Test("Loop deadline")
func loopRunDeadline() {
    var wokeUp = false
    var state: Fiber.State = .none
    fiber {
        state = sleep(until: .now.advanced(by: .seconds(-1)))
        wokeUp = true
    }
...
@Test("yield defers in the same cycle")
